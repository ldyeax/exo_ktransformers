...
def _create_inference_wrapper(
    ...
    # `swiglu_limit != 0` is meaningful only on the DSV4/MX activation paths.
    # AMXINT4 uses the same C++ act_fn and is admitted for the explicit split
    # MXFP4-GPU/AMXINT4-CPU experiment. NativeMoEWrapper
    ...
    extra_kwargs = {}
    if method in ("AMXINT4", "MXFP4", "MXFP8"):
        extra_kwargs["swiglu_limit"] = swiglu_limit
        extra_kwargs["swiglu_alpha"] = swiglu_alpha
    elif swiglu_limit != 0.0:
        raise ValueError(
            f"swiglu_limit={swiglu_limit} is only supported on method="
            f"'AMXINT4'/'MXFP4'/'MXFP8', "
            ...
            f"a clamp-capable method — either unset the env or select "
            f"AMXINT4/MXFP4/MXFP8 explicitly."
        )
    return backend_cls(
        layer_idx=layer_idx,
        num_experts=num_experts,
